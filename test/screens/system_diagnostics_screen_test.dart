// 시스템 진단 화면과 대시보드 배너의 표시 규칙을 검증합니다.
//
// 특히 "관측 불가"가 정상으로 보이지 않는지를 고정합니다. 그것이 이 화면의 존재 이유
// 중 하나입니다.
...
///
/// rosbridge를 띄우지 않고 화면 표시 규칙만 검증합니다. 주입은 provider가
/// `@visibleForTesting`으로 열어 둔 훅을 그대로 씁니다.
...
void main() {
  group('SystemDiagnosticsScreen', () {
    ...
    testWidgets('활성 결함의 컴포넌트·등급·조치를 모두 보여준다', (tester) async {
      ...
      // 문자열 한 줄이 아니라 네 가지 정보가 모두 보여야 합니다.
      // 등급은 관리자용 한국어 이름으로 보이고, 기계 판독 코드는 따로 보입니다.
    ...
    testWidgets('관측 불가가 없으면 설명 문구도 없다', (tester) async {
      ...
      // voice/app이 여전히 0이므로 설명은 남습니다. guidance만 바꿔 확인합니다.
      await tester
          .pumpWidget(wrap(const SystemDiagnosticsScreen(), supervisor));
      await tester.pump();
    ...
    testWidgets('이벤트 이력을 보여주고 reminder는 제외한다', (tester) async {
      ...
      // raised와 cleared는 남고 reminder는 빠집니다.
      // 이력 한 줄은 '<컴포넌트> · <전이>' 형태이므로 부분 일치로 확인합니다.
